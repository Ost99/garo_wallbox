...
async def async_setup_entry(hass: HomeAssistant, entry: GaroConfigEntry, async_add_entities):
    ...
    if entry.runtime_data.meter_coordinator:
        ...
        def add_lb_current_entity(
            ...
            charger_count = (
                1
                + len(coordinator.slaves)
                + (1 if configuration.has_twin else 0)
            )
            ...
            entities.append(
                GaroMeterNumberEntity(
                    ...
                    entry,
                    GaroMeterNumberEntityDescription(
                        key=f"load_balancing_current_{meter_number}",
                        translation_key=f"load_balancing_current_{meter_number}",
                        name=f"Meter {meter_number} Current Limit",
                        icon="mdi:current-ac",
                        native_max_value=charger_count * 32,
                        native_min_value=16,
                        native_step=1,
                        native_unit_of_measurement="A",
                        mode=NumberMode.SLIDER,
                        get_value=lambda _: get_fuse(),
                        set_value=set_fuse,
                        is_available=lambda: True,
                    ),
                    meter,
